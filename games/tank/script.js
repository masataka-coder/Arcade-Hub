...
const _translationsCallbacks = [];

function onTranslationsReady(cb) {
    if (_translationsReady) { cb(); } else { _translationsCallbacks.push(cb); }
}

(function loadTranslations() {
    const CACHE_KEY = 'tank_translations_cache';
    const cached = localStorage.getItem(CACHE_KEY);
    if (cached) {
        try {
            translations = JSON.parse(cached);
            _translationsReady = true;
            _translationsCallbacks.forEach(cb => cb());
            return;
    ...
    fetch('translations.json')
        ...
        .catch(() => {
            // Fallback: minimal Japanese defaults so the game still loads
            translations = { ja: { subtitle: '砲撃バトルゲーム', victory: 'VICTORY!', gameover: 'GAME OVER' }, en: { subtitle: 'Artillery Battle Game', victory: 'VICTORY!', gameover: 'GAME OVER' } };
            _translationsReady = true;
            _translationsCallbacks.forEach(cb => cb());
        });
})();
